...
CLASS zscm_tinct DEFINITION
  ...
  PUBLIC SECTION.
    ...
    TYPES:
      "! <p class="shorttext synchronized">Z_I_TINCTType</p>
      BEGIN OF tys_z_i_tincttype,
        "! <em>Key property</em> Spras
        spras  TYPE c LENGTH 2,
        "! <em>Key property</em> Inco1
        inco_1 TYPE c LENGTH 3,
        "! Bezei
        bezei  TYPE c LENGTH 30,
        "! odata.etag
        etag   TYPE string,
      END OF tys_z_i_tincttype,
      "! <p class="shorttext synchronized">List of Z_I_TINCTType</p>
      tyt_z_i_tincttype TYPE STANDARD TABLE OF tys_z_i_tincttype WITH DEFAULT KEY.
    ...
    CONSTANTS:
      ...
      BEGIN OF gcs_entity_set,
        ...
        "! Z_I_TINCT
        "! <br/> Collection of type 'Z_I_TINCTType'
        z_i_tinct                  TYPE /iwbep/if_cp_runtime_types=>ty_entity_set_name VALUE 'Z_I_TINCT',
    ...
    CONSTANTS:
      ...
      BEGIN OF gcs_entity_type,
        ...
        "! <p class="shorttext synchronized">Internal names for Z_I_TINCTType</p>
        "! See also structure type {@link ..tys_z_i_tincttype}
        BEGIN OF z_i_tincttype,
          "! <p class="shorttext synchronized">Navigation properties</p>
          BEGIN OF navigation,
            "! Dummy field - Structure must not be empty
            dummy TYPE int1 VALUE 0,
          END OF navigation,
        END OF z_i_tincttype,
  ...
  PRIVATE SECTION.
    ...
    "! <p class="shorttext synchronized">Define Z_I_TINCTType</p>
    "! @raising /iwbep/cx_gateway | <p class="shorttext synchronized">Gateway Exception</p>
    METHODS def_z_i_tincttype RAISING /iwbep/cx_gateway.

ENDCLASS.



CLASS ZSCM_TINCT IMPLEMENTATION.


  METHOD /iwbep/if_v4_mp_basic_pm~define.

    mo_model = io_model.
    mo_model->set_schema_namespace( 'cds_zsd_tinct' ) ##NO_TEXT.
  ...
  METHOD def_z_i_tincttype.
    ...
    lo_entity_type = mo_model->create_entity_type_by_struct(
                                    iv_entity_type_name       = 'Z_I_TINCTTYPE'
                                    is_structure              = VALUE tys_z_i_tincttype( )
                                    iv_do_gen_prim_props         = abap_true
                                    iv_do_gen_prim_prop_colls    = abap_true
                                    iv_do_add_conv_to_prim_props = abap_true ).

    lo_entity_type->set_edm_name( 'Z_I_TINCTType' ) ##NO_TEXT.


    lo_entity_set = lo_entity_type->create_entity_set( 'Z_I_TINCT' ).
    lo_entity_set->set_edm_name( 'Z_I_TINCT' ) ##NO_TEXT.
    ...
    lo_primitive_property = lo_entity_type->get_primitive_property( 'BEZEI' ).
    lo_primitive_property->set_edm_name( 'Bezei' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 30 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).
