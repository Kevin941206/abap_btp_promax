...
CLASS zscm_tcurt DEFINITION
  ...
  PUBLIC SECTION.
    ...
    TYPES:
      "! <p class="shorttext synchronized">Z_I_TCURTType</p>
      BEGIN OF tys_z_i_tcurttype,
        "! Delete_mc
        ...
        "! Ktext
        ktext     TYPE c LENGTH 15,
      END OF tys_z_i_tcurttype,
      "! <p class="shorttext synchronized">List of Z_I_TCURTType</p>
      tyt_z_i_tcurttype TYPE STANDARD TABLE OF tys_z_i_tcurttype WITH DEFAULT KEY.
    ...
    CONSTANTS:
      ...
      BEGIN OF gcs_entity_set,
        ...
        "! Z_I_TCURT
        "! <br/> Collection of type 'Z_I_TCURTType'
        z_i_tcurt                  TYPE /iwbep/if_cp_runtime_types=>ty_entity_set_name VALUE 'Z_I_TCURT',
    ...
    CONSTANTS:
      ...
      BEGIN OF gcs_entity_type,
        ...
        "! <p class="shorttext synchronized">Internal names for Z_I_TCURTType</p>
        "! See also structure type {@link ..tys_z_i_tcurttype}
        BEGIN OF z_i_tcurttype,
          "! <p class="shorttext synchronized">Navigation properties</p>
          BEGIN OF navigation,
            "! Dummy field - Structure must not be empty
            dummy TYPE int1 VALUE 0,
          END OF navigation,
        END OF z_i_tcurttype,
  ...
  PRIVATE SECTION.
    ...
    "! <p class="shorttext synchronized">Define Z_I_TCURTType</p>
    "! @raising /iwbep/cx_gateway | <p class="shorttext synchronized">Gateway Exception</p>
    METHODS def_z_i_tcurttype RAISING /iwbep/cx_gateway.

ENDCLASS.



CLASS ZSCM_TCURT IMPLEMENTATION.


  METHOD /iwbep/if_v4_mp_basic_pm~define.

    mo_model = io_model.
    mo_model->set_schema_namespace( 'cds_zsd_tcurt' ) ##NO_TEXT.
  ...
  METHOD def_z_i_tcurttype.
    ...
    lo_entity_type = mo_model->create_entity_type_by_struct(
                                    iv_entity_type_name       = 'Z_I_TCURTTYPE'
                                    is_structure              = VALUE tys_z_i_tcurttype( )
                                    iv_do_gen_prim_props         = abap_true
                                    iv_do_gen_prim_prop_colls    = abap_true
                                    iv_do_add_conv_to_prim_props = abap_true ).

    lo_entity_type->set_edm_name( 'Z_I_TCURTType' ) ##NO_TEXT.


    lo_entity_set = lo_entity_type->create_entity_set( 'Z_I_TCURT' ).
    lo_entity_set->set_edm_name( 'Z_I_TCURT' ) ##NO_TEXT.
