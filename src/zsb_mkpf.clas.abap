...
CLASS zsb_mkpf DEFINITION
  ...
  PUBLIC SECTION.
    ...
    TYPES:
      "! <p class="shorttext synchronized">Z_I_MKPFType</p>
      BEGIN OF tys_z_i_mkpftype,
        "! Delete_mc
        ...
        "! Wever
        wever          TYPE c LENGTH 1,
        "! Xabln
        xabln          TYPE c LENGTH 10,
        "! Awsys
        awsys          TYPE c LENGTH 10,
        "! Bla2d
        bla_2_d        TYPE c LENGTH 2,
        "! Tcode2
        tcode_2        TYPE c LENGTH 20,
        "! Bfwms
        bfwms          TYPE c LENGTH 1,
        "! Exnum
        exnum          TYPE c LENGTH 10,
        "! SpeBudatUhr
        spe_budat_uhr  TYPE timn,
        "! SpeBudatZone
        spe_budat_zone TYPE c LENGTH 6,
        "! LeVbeln
        le_vbeln       TYPE c LENGTH 10,
        "! SpeLogsys
        spe_logsys     TYPE c LENGTH 10,
        "! SpeMdnumEwm
        spe_mdnum_ewm  TYPE c LENGTH 16,
        "! GtsCusrefNo
        gts_cusref_no  TYPE c LENGTH 35,
        "! FlsRsto
        ...
        "! Xcompl
        xcompl         TYPE c LENGTH 1,
      END OF tys_z_i_mkpftype,
      "! <p class="shorttext synchronized">List of Z_I_MKPFType</p>
      tyt_z_i_mkpftype TYPE STANDARD TABLE OF tys_z_i_mkpftype WITH DEFAULT KEY.
    ...
    CONSTANTS:
      ...
      BEGIN OF gcs_entity_set,
        ...
        "! Z_I_MKPF
        "! <br/> Collection of type 'Z_I_MKPFType'
        z_i_mkpf                   TYPE /iwbep/if_cp_runtime_types=>ty_entity_set_name VALUE 'Z_I_MKPF',
    ...
    CONSTANTS:
      ...
      BEGIN OF gcs_entity_type,
        ...
        "! <p class="shorttext synchronized">Internal names for Z_I_MKPFType</p>
        "! See also structure type {@link ..tys_z_i_mkpftype}
        BEGIN OF z_i_mkpftype,
          "! <p class="shorttext synchronized">Navigation properties</p>
          BEGIN OF navigation,
            "! Dummy field - Structure must not be empty
            dummy TYPE int1 VALUE 0,
  ...
  PRIVATE SECTION.
    ...
    "! <p class="shorttext synchronized">Define Z_I_MKPFType</p>
    "! @raising /iwbep/cx_gateway | <p class="shorttext synchronized">Gateway Exception</p>
    METHODS def_z_i_mkpftype RAISING /iwbep/cx_gateway.

ENDCLASS.



CLASS ZSB_MKPF IMPLEMENTATION.


  METHOD /iwbep/if_v4_mp_basic_pm~define.

    mo_model = io_model.
    mo_model->set_schema_namespace( 'cds_zsd_mkpf' ) ##NO_TEXT.
  ...
  METHOD def_z_i_mkpftype.
    ...
    lo_entity_type = mo_model->create_entity_type_by_struct(
                                    iv_entity_type_name       = 'Z_I_MKPFTYPE'
                                    is_structure              = VALUE tys_z_i_mkpftype( )
                                    iv_do_gen_prim_props         = abap_true
                                    iv_do_gen_prim_prop_colls    = abap_true
                                    iv_do_add_conv_to_prim_props = abap_true ).

    lo_entity_type->set_edm_name( 'Z_I_MKPFType' ) ##NO_TEXT.


    lo_entity_set = lo_entity_type->create_entity_set( 'Z_I_MKPF' ).
    lo_entity_set->set_edm_name( 'Z_I_MKPF' ) ##NO_TEXT.
    ...
    lo_primitive_property = lo_entity_type->get_primitive_property( 'BLAUM' ).
    lo_primitive_property->set_edm_name( 'Blaum' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 2 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).
    ...
    lo_primitive_property = lo_entity_type->get_primitive_property( 'WEVER' ).
    lo_primitive_property->set_edm_name( 'Wever' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    ...
    lo_primitive_property = lo_entity_type->get_primitive_property( 'BLA_2_D' ).
    lo_primitive_property->set_edm_name( 'Bla2d' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 2 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'TCODE_2' ).
    lo_primitive_property->set_edm_name( 'Tcode2' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 20 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'BFWMS' ).
    lo_primitive_property->set_edm_name( 'Bfwms' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 1 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).
    ...
    lo_primitive_property = lo_entity_type->get_primitive_property( 'SPE_BUDAT_UHR' ).
    lo_primitive_property->set_edm_name( 'SpeBudatUhr' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'TimeOfDay' ) ##NO_TEXT.
    lo_primitive_property->set_is_nullable( ).
    lo_primitive_property->set_edm_type_v2( 'Time' ) ##NO_TEXT.

    lo_primitive_property = lo_entity_type->get_primitive_property( 'SPE_BUDAT_ZONE' ).
    lo_primitive_property->set_edm_name( 'SpeBudatZone' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 6 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'LE_VBELN' ).
    lo_primitive_property->set_edm_name( 'LeVbeln' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 10 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'SPE_LOGSYS' ).
    lo_primitive_property->set_edm_name( 'SpeLogsys' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 10 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'SPE_MDNUM_EWM' ).
    lo_primitive_property->set_edm_name( 'SpeMdnumEwm' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 16 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'GTS_CUSREF_NO' ).
    lo_primitive_property->set_edm_name( 'GtsCusrefNo' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    ...
    lo_primitive_property = lo_entity_type->get_primitive_property( 'XCOMPL' ).
    lo_primitive_property->set_edm_name( 'Xcompl' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 1 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).
