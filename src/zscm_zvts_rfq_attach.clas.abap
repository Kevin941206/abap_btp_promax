...
"! <em>cds_zsd_zvts_rfq_attach</em>
CLASS zscm_zvts_rfq_attach DEFINITION
  PUBLIC
  ...
  PUBLIC SECTION.
    ...
    TYPES:
      "! <p class="shorttext synchronized">Z_I_SOFFPHIOType</p>
      BEGIN OF tys_z_i_soffphiotype,
        "! <em>Key property</em> PhioId
        phio_id    TYPE c LENGTH 32,
        "! PhClass
        ph_class   TYPE c LENGTH 10,
        "! LoioId
        loio_id    TYPE c LENGTH 32,
        "! LoClass
        lo_class   TYPE c LENGTH 10,
        "! State
        state      TYPE c LENGTH 4,
        "! Descript
        descript   TYPE c LENGTH 64,
        "! Langu
        langu      TYPE c LENGTH 2,
        "! CreaUser
        crea_user  TYPE c LENGTH 12,
        "! CreaTime
        crea_time  TYPE c LENGTH 14,
        "! ChngUser
        chng_user  TYPE c LENGTH 12,
        "! ChngTime
        chng_time  TYPE c LENGTH 14,
        "! StorCat
        stor_cat   TYPE c LENGTH 10,
        "! DocFormat
        doc_format TYPE c LENGTH 4,
        "! DocProt
        doc_prot   TYPE c LENGTH 4,
        "! Prop01
        prop_01    TYPE c LENGTH 15,
        "! Prop02
        prop_02    TYPE c LENGTH 15,
        "! Prop03
        prop_03    TYPE c LENGTH 15,
        "! Prop04
        prop_04    TYPE c LENGTH 15,
        "! Prop05
        prop_05    TYPE c LENGTH 20,
        "! Prop06
        prop_06    TYPE c LENGTH 20,
        "! Prop07
        prop_07    TYPE c LENGTH 20,
        "! Prop08
        prop_08    TYPE c LENGTH 40,
        "! Prop09
        prop_09    TYPE c LENGTH 40,
        "! Prop10
        prop_10    TYPE c LENGTH 40,
      END OF tys_z_i_soffphiotype,
      "! <p class="shorttext synchronized">List of Z_I_SOFFPHIOType</p>
      tyt_z_i_soffphiotype TYPE STANDARD TABLE OF tys_z_i_soffphiotype WITH DEFAULT KEY.

    TYPES:
      "! <p class="shorttext synchronized">Z_I_SRGBTBRELType</p>
      BEGIN OF tys_z_i_srgbtbreltype,
      ...
      END OF tys_z_i_srgbtbreltype,
      "! <p class="shorttext synchronized">List of Z_I_SRGBTBRELType</p>
      tyt_z_i_srgbtbreltype TYPE STANDARD TABLE OF tys_z_i_srgbtbreltype WITH DEFAULT KEY.
    ...
    CONSTANTS:
      ...
      BEGIN OF gcs_entity_set,
        ...
        "! Z_I_SOFFPHIO
        "! <br/> Collection of type 'Z_I_SOFFPHIOType'
        z_i_soffphio               TYPE /iwbep/if_cp_runtime_types=>ty_entity_set_name VALUE 'Z_I_SOFFPHIO',
        "! Z_I_SRGBTBREL
    ...
    CONSTANTS:
      ...
      BEGIN OF gcs_entity_type,
        ...
        "! <p class="shorttext synchronized">Internal names for Z_I_SOFFPHIOType</p>
        "! See also structure type {@link ..tys_z_i_soffphiotype}
        BEGIN OF z_i_soffphiotype,
          "! <p class="shorttext synchronized">Navigation properties</p>
          BEGIN OF navigation,
            "! Dummy field - Structure must not be empty
            dummy TYPE int1 VALUE 0,
          END OF navigation,
        END OF z_i_soffphiotype,
        "! <p class="shorttext synchronized">Internal names for Z_I_SRGBTBRELType</p>
        "! See also structure type {@link ..tys_z_i_srgbtbreltype}
        BEGIN OF z_i_srgbtbreltype,
          "! <p class="shorttext synchronized">Navigation properties</p>
          BEGIN OF navigation,
            "! Dummy field - Structure must not be empty
            dummy TYPE int1 VALUE 0,
          END OF navigation,
        END OF z_i_srgbtbreltype,
  ...
  PRIVATE SECTION.
    ...
    "! <p class="shorttext synchronized">Define Z_I_SOFFPHIOType</p>
    "! @raising /iwbep/cx_gateway | <p class="shorttext synchronized">Gateway Exception</p>
    METHODS def_z_i_soffphiotype RAISING /iwbep/cx_gateway.

    "! <p class="shorttext synchronized">Define Z_I_SRGBTBRELType</p>
    "! @raising /iwbep/cx_gateway | <p class="shorttext synchronized">Gateway Exception</p>
    METHODS def_z_i_srgbtbreltype RAISING /iwbep/cx_gateway.

ENDCLASS.



CLASS ZSCM_ZVTS_RFQ_ATTACH IMPLEMENTATION.


  METHOD /iwbep/if_v4_mp_basic_pm~define.

    mo_model = io_model.
    mo_model->set_schema_namespace( 'cds_zsd_zvts_rfq_attach' ) ##NO_TEXT.
    ...
    def_sap_value_help( ).
    def_z_i_soffphiotype( ).
    def_z_i_srgbtbreltype( ).
  ...
  METHOD def_z_i_soffphiotype.
    ...
    lo_entity_type = mo_model->create_entity_type_by_struct(
                                    iv_entity_type_name       = 'Z_I_SOFFPHIOTYPE'
                                    is_structure              = VALUE tys_z_i_soffphiotype( )
                                    iv_do_gen_prim_props         = abap_true
                                    iv_do_gen_prim_prop_colls    = abap_true
                                    iv_do_add_conv_to_prim_props = abap_true ).

    lo_entity_type->set_edm_name( 'Z_I_SOFFPHIOType' ) ##NO_TEXT.


    lo_entity_set = lo_entity_type->create_entity_set( 'Z_I_SOFFPHIO' ).
    lo_entity_set->set_edm_name( 'Z_I_SOFFPHIO' ) ##NO_TEXT.


    lo_primitive_property = lo_entity_type->get_primitive_property( 'PHIO_ID' ).
    lo_primitive_property->set_edm_name( 'PhioId' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 32 ) ##NUMBER_OK.
    lo_primitive_property->set_is_key( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'PH_CLASS' ).
    lo_primitive_property->set_edm_name( 'PhClass' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 10 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'LOIO_ID' ).
    lo_primitive_property->set_edm_name( 'LoioId' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 32 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'LO_CLASS' ).
    lo_primitive_property->set_edm_name( 'LoClass' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 10 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).
    ...
    lo_primitive_property = lo_entity_type->get_primitive_property( 'CREA_TIME' ).
    lo_primitive_property->set_edm_name( 'CreaTime' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 14 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'CHNG_USER' ).
    lo_primitive_property->set_edm_name( 'ChngUser' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 12 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'CHNG_TIME' ).
    lo_primitive_property->set_edm_name( 'ChngTime' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 14 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'STOR_CAT' ).
    lo_primitive_property->set_edm_name( 'StorCat' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 10 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'DOC_FORMAT' ).
    lo_primitive_property->set_edm_name( 'DocFormat' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 4 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'DOC_PROT' ).
    lo_primitive_property->set_edm_name( 'DocProt' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 4 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'PROP_01' ).
    lo_primitive_property->set_edm_name( 'Prop01' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 15 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'PROP_02' ).
    lo_primitive_property->set_edm_name( 'Prop02' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 15 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'PROP_03' ).
    lo_primitive_property->set_edm_name( 'Prop03' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 15 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'PROP_04' ).
    lo_primitive_property->set_edm_name( 'Prop04' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 15 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'PROP_05' ).
    lo_primitive_property->set_edm_name( 'Prop05' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 20 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'PROP_06' ).
    lo_primitive_property->set_edm_name( 'Prop06' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 20 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'PROP_07' ).
    lo_primitive_property->set_edm_name( 'Prop07' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 20 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'PROP_08' ).
    lo_primitive_property->set_edm_name( 'Prop08' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 40 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'PROP_09' ).
    lo_primitive_property->set_edm_name( 'Prop09' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 40 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'PROP_10' ).
    lo_primitive_property->set_edm_name( 'Prop10' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 40 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).
  ...
  METHOD def_z_i_srgbtbreltype.
    ...
    lo_entity_type = mo_model->create_entity_type_by_struct(
                                    iv_entity_type_name       = 'Z_I_SRGBTBRELTYPE'
                                    is_structure              = VALUE tys_z_i_srgbtbreltype( )
                                    iv_do_gen_prim_props         = abap_true
                                    iv_do_gen_prim_prop_colls    = abap_true
                                    iv_do_add_conv_to_prim_props = abap_true ).
