...
CLASS zscm_vttp DEFINITION
  ...
  PUBLIC SECTION.
    ...
    TYPES:
      "! <p class="shorttext synchronized">z_i_vttpType</p>
      BEGIN OF tys_z_i_vttp_type,
        "! <em>Key property</em> Vbeln
        vbeln   TYPE c LENGTH 10,
        "! Tprfo
        tprfo   TYPE c LENGTH 4,
        "! Ernam
        ...
        erzet   TYPE timn,
        "! Pksta
        pksta   TYPE abap_bool,
        "! Kzhulfg
        kzhulfg TYPE abap_bool,
      END OF tys_z_i_vttp_type,
      "! <p class="shorttext synchronized">List of z_i_vttpType</p>
      tyt_z_i_vttp_type TYPE STANDARD TABLE OF tys_z_i_vttp_type WITH DEFAULT KEY.
    ...
    CONSTANTS:
      ...
      BEGIN OF gcs_entity_set,
        ...
        "! z_i_vttp
        "! <br/> Collection of type 'z_i_vttpType'
        z_i_vttp                   TYPE /iwbep/if_cp_runtime_types=>ty_entity_set_name VALUE 'Z_I_VTTP',
    ...
    CONSTANTS:
      ...
      BEGIN OF gcs_entity_type,
        ...
        "! <p class="shorttext synchronized">Internal names for z_i_vttpType</p>
        "! See also structure type {@link ..tys_z_i_vttp_type}
        BEGIN OF z_i_vttp_type,
          "! <p class="shorttext synchronized">Navigation properties</p>
          BEGIN OF navigation,
            "! Dummy field - Structure must not be empty
            dummy TYPE int1 VALUE 0,
          END OF navigation,
        END OF z_i_vttp_type,
  ...
  PRIVATE SECTION.
    ...
    "! <p class="shorttext synchronized">Define z_i_vttpType</p>
    "! @raising /iwbep/cx_gateway | <p class="shorttext synchronized">Gateway Exception</p>
    METHODS def_z_i_vttp_type RAISING /iwbep/cx_gateway.

ENDCLASS.


CLASS zscm_vttp IMPLEMENTATION.

  METHOD /iwbep/if_v4_mp_basic_pm~define.

    mo_model = io_model.
    mo_model->set_schema_namespace( 'cds_zsd_vttp' ) ##NO_TEXT.
  ...
  METHOD def_z_i_vttp_type.
    ...
    lo_entity_type = mo_model->create_entity_type_by_struct(
                                    iv_entity_type_name       = 'Z_I_VTTP_TYPE'
                                    is_structure              = VALUE tys_z_i_vttp_type( )
                                    iv_do_gen_prim_props         = abap_true
                                    iv_do_gen_prim_prop_colls    = abap_true
                                    iv_do_add_conv_to_prim_props = abap_true ).

    lo_entity_type->set_edm_name( 'z_i_vttpType' ) ##NO_TEXT.


    lo_entity_set = lo_entity_type->create_entity_set( 'Z_I_VTTP' ).
    lo_entity_set->set_edm_name( 'z_i_vttp' ) ##NO_TEXT.
    ...
    lo_primitive_property = lo_entity_type->get_primitive_property( 'TPRFO' ).
    lo_primitive_property->set_edm_name( 'Tprfo' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    ...
    lo_primitive_property = lo_entity_type->get_primitive_property( 'PKSTA' ).
    lo_primitive_property->set_edm_name( 'Pksta' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'Boolean' ) ##NO_TEXT.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'KZHULFG' ).
    lo_primitive_property->set_edm_name( 'Kzhulfg' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'Boolean' ) ##NO_TEXT.
    lo_primitive_property->set_is_nullable( ).
