...
"! <em>cds_zsd_mara_makt</em>
CLASS zscm_mara_makt DEFINITION
  PUBLIC
  ...
  PUBLIC SECTION.
    ...
    TYPES:
      "! <p class="shorttext synchronized">Z_I_MARA_MAKTType</p>
      BEGIN OF tys_z_i_mara_makttype,
        ...
        "! odata.etag
        etag  TYPE string,
      END OF tys_z_i_mara_makttype,
      "! <p class="shorttext synchronized">List of Z_I_MARA_MAKTType</p>
      tyt_z_i_mara_makttype TYPE STANDARD TABLE OF tys_z_i_mara_makttype WITH DEFAULT KEY.
    ...
    CONSTANTS:
      ...
      BEGIN OF gcs_entity_set,
        ...
        "! Z_I_MARA_MAKT
        "! <br/> Collection of type 'Z_I_MARA_MAKTType'
        z_i_mara_makt              TYPE /iwbep/if_cp_runtime_types=>ty_entity_set_name VALUE 'Z_I_MARA_MAKT',
    ...
    CONSTANTS:
      ...
      BEGIN OF gcs_entity_type,
        ...
        "! <p class="shorttext synchronized">Internal names for Z_I_MARA_MAKTType</p>
        "! See also structure type {@link ..tys_z_i_mara_makttype}
        BEGIN OF z_i_mara_makttype,
          "! <p class="shorttext synchronized">Navigation properties</p>
          BEGIN OF navigation,
            "! Dummy field - Structure must not be empty
            dummy TYPE int1 VALUE 0,
          END OF navigation,
        END OF z_i_mara_makttype,
  ...
  PRIVATE SECTION.
    ...
    "! <p class="shorttext synchronized">Define Z_I_MARA_MAKTType</p>
    "! @raising /iwbep/cx_gateway | <p class="shorttext synchronized">Gateway Exception</p>
    METHODS def_z_i_mara_makttype RAISING /iwbep/cx_gateway.

ENDCLASS.



CLASS ZSCM_MARA_MAKT IMPLEMENTATION.


  METHOD /iwbep/if_v4_mp_basic_pm~define.

    mo_model = io_model.
    mo_model->set_schema_namespace( 'cds_zsd_mara_makt' ) ##NO_TEXT.
  ...
  METHOD def_z_i_mara_makttype.
    ...
    lo_entity_type = mo_model->create_entity_type_by_struct(
                                    iv_entity_type_name       = 'Z_I_MARA_MAKTTYPE'
                                    is_structure              = VALUE tys_z_i_mara_makttype( )
                                    iv_do_gen_prim_props         = abap_true
                                    iv_do_gen_prim_prop_colls    = abap_true
                                    iv_do_add_conv_to_prim_props = abap_true ).

    lo_entity_type->set_edm_name( 'Z_I_MARA_MAKTType' ) ##NO_TEXT.


    lo_entity_set = lo_entity_type->create_entity_set( 'Z_I_MARA_MAKT' ).
    lo_entity_set->set_edm_name( 'Z_I_MARA_MAKT' ) ##NO_TEXT.
    ...
    lo_primitive_property = lo_entity_type->get_primitive_property( 'MAKTX' ).
    lo_primitive_property->set_edm_name( 'Maktx' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 40 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'MAKTG' ).
    lo_primitive_property->set_edm_name( 'Maktg' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 40 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).
