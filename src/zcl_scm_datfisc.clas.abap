...
"! <em>cds_zsd_datfisc</em>
CLASS zcl_scm_datfisc DEFINITION
  PUBLIC
  ...
  PUBLIC SECTION.
    ...
    TYPES:
      "! <p class="shorttext synchronized">Z_I_DATFISCType</p>
      BEGIN OF tys_z_i_datfisctype,
        "! <em>Key property</em> Uuid
        uuid   TYPE c LENGTH 50,
        "! Bukrs
        ...
        "! Xref1
        xref_1 TYPE c LENGTH 20,
        "! Forpa
        forpa  TYPE c LENGTH 45,
        "! Metpa
        metpa  TYPE c LENGTH 4,
        "! Rfc
        rfc    TYPE c LENGTH 15,
      END OF tys_z_i_datfisctype,
      "! <p class="shorttext synchronized">List of Z_I_DATFISCType</p>
      tyt_z_i_datfisctype TYPE STANDARD TABLE OF tys_z_i_datfisctype WITH DEFAULT KEY.
    ...
    CONSTANTS:
      ...
      BEGIN OF gcs_entity_set,
        ...
        "! Z_I_DATFISC
        "! <br/> Collection of type 'Z_I_DATFISCType'
        z_i_datfisc                TYPE /iwbep/if_cp_runtime_types=>ty_entity_set_name VALUE 'Z_I_DATFISC',
    ...
    CONSTANTS:
      ...
      BEGIN OF gcs_entity_type,
        ...
        "! <p class="shorttext synchronized">Internal names for Z_I_DATFISCType</p>
        "! See also structure type {@link ..tys_z_i_datfisctype}
        BEGIN OF z_i_datfisctype,
          "! <p class="shorttext synchronized">Navigation properties</p>
          BEGIN OF navigation,
            "! Dummy field - Structure must not be empty
            dummy TYPE int1 VALUE 0,
          END OF navigation,
        END OF z_i_datfisctype,
  ...
  PRIVATE SECTION.
    ...
    "! <p class="shorttext synchronized">Define Z_I_DATFISCType</p>
    "! @raising /iwbep/cx_gateway | <p class="shorttext synchronized">Gateway Exception</p>
    METHODS def_z_i_datfisctype RAISING /iwbep/cx_gateway.

ENDCLASS.



CLASS ZCL_SCM_DATFISC IMPLEMENTATION.


  METHOD /iwbep/if_v4_mp_basic_pm~define.

    mo_model = io_model.
    mo_model->set_schema_namespace( 'cds_zsd_datfisc' ) ##NO_TEXT.
  ...
  METHOD def_z_i_datfisctype.
    ...
    lo_entity_type = mo_model->create_entity_type_by_struct(
                                    iv_entity_type_name       = 'Z_I_DATFISCTYPE'
                                    is_structure              = VALUE tys_z_i_datfisctype( )
                                    iv_do_gen_prim_props         = abap_true
                                    iv_do_gen_prim_prop_colls    = abap_true
                                    iv_do_add_conv_to_prim_props = abap_true ).

    lo_entity_type->set_edm_name( 'Z_I_DATFISCType' ) ##NO_TEXT.


    lo_entity_set = lo_entity_type->create_entity_set( 'Z_I_DATFISC' ).
    lo_entity_set->set_edm_name( 'Z_I_DATFISC' ) ##NO_TEXT.
    ...
    lo_primitive_property = lo_entity_type->get_primitive_property( 'FORPA' ).
    lo_primitive_property->set_edm_name( 'Forpa' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 45 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'METPA' ).
    lo_primitive_property->set_edm_name( 'Metpa' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 4 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'RFC' ).
    lo_primitive_property->set_edm_name( 'Rfc' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 15 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).
