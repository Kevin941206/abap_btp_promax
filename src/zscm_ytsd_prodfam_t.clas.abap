...
"! <em>cds_zsd_ytsd_prodfam_t</em>
CLASS zscm_ytsd_prodfam_t DEFINITION
  PUBLIC
  ...
  PUBLIC SECTION.
    ...
    TYPES:
      "! <p class="shorttext synchronized">Z_I_YTSD_PRODFAM_TType</p>
      BEGIN OF tys_z_i_ytsd_prodfam_ttype,
        "! Delete_mc
        delete_mc    TYPE abap_bool,
        "! Update_mc
        update_mc    TYPE abap_bool,
        "! <em>Key property</em> Subfamily
        subfamily    TYPE c LENGTH 1,
        "! SubfamTxtE
        subfam_txt_e TYPE c LENGTH 30,
        "! SubfamTxtS
        subfam_txt_s TYPE c LENGTH 30,
      END OF tys_z_i_ytsd_prodfam_ttype,
      "! <p class="shorttext synchronized">List of Z_I_YTSD_PRODFAM_TType</p>
      tyt_z_i_ytsd_prodfam_ttype TYPE STANDARD TABLE OF tys_z_i_ytsd_prodfam_ttype WITH DEFAULT KEY.
    ...
    CONSTANTS:
      ...
      BEGIN OF gcs_entity_set,
        ...
        "! Z_I_YTSD_PRODFAM_T
        "! <br/> Collection of type 'Z_I_YTSD_PRODFAM_TType'
        z_i_ytsd_prodfam_t         TYPE /iwbep/if_cp_runtime_types=>ty_entity_set_name VALUE 'Z_I_YTSD_PRODFAM_T',
    ...
    CONSTANTS:
      ...
      BEGIN OF gcs_entity_type,
        ...
        "! <p class="shorttext synchronized">Internal names for Z_I_YTSD_PRODFAM_TType</p>
        "! See also structure type {@link ..tys_z_i_ytsd_prodfam_ttype}
        BEGIN OF z_i_ytsd_prodfam_ttype,
          "! <p class="shorttext synchronized">Navigation properties</p>
          BEGIN OF navigation,
            "! Dummy field - Structure must not be empty
            dummy TYPE int1 VALUE 0,
          END OF navigation,
        END OF z_i_ytsd_prodfam_ttype,
  ...
  PRIVATE SECTION.
    ...
    "! <p class="shorttext synchronized">Define Z_I_YTSD_PRODFAM_TType</p>
    "! @raising /iwbep/cx_gateway | <p class="shorttext synchronized">Gateway Exception</p>
    METHODS def_z_i_ytsd_prodfam_ttype RAISING /iwbep/cx_gateway.

ENDCLASS.



CLASS ZSCM_YTSD_PRODFAM_T IMPLEMENTATION.


  METHOD /iwbep/if_v4_mp_basic_pm~define.

    mo_model = io_model.
    mo_model->set_schema_namespace( 'cds_zsd_ytsd_prodfam_t' ) ##NO_TEXT.
  ...
  METHOD def_z_i_ytsd_prodfam_ttype.
    ...
    lo_entity_type = mo_model->create_entity_type_by_struct(
                                    iv_entity_type_name       = 'Z_I_YTSD_PRODFAM_TTYPE'
                                    is_structure              = VALUE tys_z_i_ytsd_prodfam_ttype( )
                                    iv_do_gen_prim_props         = abap_true
                                    iv_do_gen_prim_prop_colls    = abap_true
                                    iv_do_add_conv_to_prim_props = abap_true ).

    lo_entity_type->set_edm_name( 'Z_I_YTSD_PRODFAM_TType' ) ##NO_TEXT.


    lo_entity_set = lo_entity_type->create_entity_set( 'Z_I_YTSD_PRODFAM_T' ).
    lo_entity_set->set_edm_name( 'Z_I_YTSD_PRODFAM_T' ) ##NO_TEXT.
    ...
    lo_primitive_property = lo_entity_type->get_primitive_property( 'SUBFAMILY' ).
    lo_primitive_property->set_edm_name( 'Subfamily' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 1 ) ##NUMBER_OK.
    lo_primitive_property->set_is_key( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'SUBFAM_TXT_E' ).
    lo_primitive_property->set_edm_name( 'SubfamTxtE' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 30 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'SUBFAM_TXT_S' ).
    lo_primitive_property->set_edm_name( 'SubfamTxtS' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 30 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).
