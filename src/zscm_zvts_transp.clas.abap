...
"! <em>cds_zsd_zvts_transp</em>
CLASS zscm_zvts_transp DEFINITION
  PUBLIC
  ...
  PUBLIC SECTION.
    ...
    TYPES:
      "! <p class="shorttext synchronized">Z_I_ZVTS_TRANSPType</p>
      BEGIN OF tys_z_i_zvts_transptype,
        "! <em>Key property</em> Traty
        ...
        "! Sortf
        sortf   TYPE c LENGTH 4,
        "! Werfd
        werfd   TYPE c LENGTH 1,
        "! Veltp
        veltp   TYPE c LENGTH 1,
        "! Gruvs
        gruvs   TYPE c LENGTH 3,
        "! Nrverg
        nrverg  TYPE c LENGTH 1,
        "! Sscc18
        sscc_18 TYPE c LENGTH 1,
        "! IntNkr
        int_nkr TYPE c LENGTH 2,
        "! ExtNkr
        ext_nkr TYPE c LENGTH 2,
        "! Taravar
        taravar TYPE abap_bool,
        "! Stsma
        stsma   TYPE c LENGTH 8,
        "! Kzpgn
        kzpgn   TYPE c LENGTH 1,
        "! Vtext
        vtext   TYPE c LENGTH 20,
        "! odata.etag
        etag    TYPE string,
      END OF tys_z_i_zvts_transptype,
      "! <p class="shorttext synchronized">List of Z_I_ZVTS_TRANSPType</p>
      tyt_z_i_zvts_transptype TYPE STANDARD TABLE OF tys_z_i_zvts_transptype WITH DEFAULT KEY.
    ...
    CONSTANTS:
      ...
      BEGIN OF gcs_entity_set,
        ...
        "! Z_I_ZVTS_TRANSP
        "! <br/> Collection of type 'Z_I_ZVTS_TRANSPType'
        z_i_zvts_transp            TYPE /iwbep/if_cp_runtime_types=>ty_entity_set_name VALUE 'Z_I_ZVTS_TRANSP',
    ...
    CONSTANTS:
      ...
      BEGIN OF gcs_entity_type,
        ...
        "! <p class="shorttext synchronized">Internal names for Z_I_ZVTS_TRANSPType</p>
        "! See also structure type {@link ..tys_z_i_zvts_transptype}
        BEGIN OF z_i_zvts_transptype,
          "! <p class="shorttext synchronized">Navigation properties</p>
          BEGIN OF navigation,
            "! Dummy field - Structure must not be empty
            dummy TYPE int1 VALUE 0,
          END OF navigation,
        END OF z_i_zvts_transptype,
  ...
  PRIVATE SECTION.
    ...
    "! <p class="shorttext synchronized">Define Z_I_ZVTS_TRANSPType</p>
    "! @raising /iwbep/cx_gateway | <p class="shorttext synchronized">Gateway Exception</p>
    METHODS def_z_i_zvts_transptype RAISING /iwbep/cx_gateway.

ENDCLASS.



CLASS ZSCM_ZVTS_TRANSP IMPLEMENTATION.


  METHOD /iwbep/if_v4_mp_basic_pm~define.

    mo_model = io_model.
    mo_model->set_schema_namespace( 'cds_zsd_zvts_transp' ) ##NO_TEXT.
  ...
  METHOD def_z_i_zvts_transptype.
    ...
    lo_entity_type = mo_model->create_entity_type_by_struct(
                                    iv_entity_type_name       = 'Z_I_ZVTS_TRANSPTYPE'
                                    is_structure              = VALUE tys_z_i_zvts_transptype( )
                                    iv_do_gen_prim_props         = abap_true
                                    iv_do_gen_prim_prop_colls    = abap_true
                                    iv_do_add_conv_to_prim_props = abap_true ).

    lo_entity_type->set_edm_name( 'Z_I_ZVTS_TRANSPType' ) ##NO_TEXT.


    lo_entity_set = lo_entity_type->create_entity_set( 'Z_I_ZVTS_TRANSP' ).
    lo_entity_set->set_edm_name( 'Z_I_ZVTS_TRANSP' ) ##NO_TEXT.
    ...
    lo_primitive_property = lo_entity_type->get_primitive_property( 'SORTF' ).
    lo_primitive_property->set_edm_name( 'Sortf' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 4 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'WERFD' ).
    lo_primitive_property->set_edm_name( 'Werfd' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 1 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'VELTP' ).
    lo_primitive_property->set_edm_name( 'Veltp' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 1 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'GRUVS' ).
    lo_primitive_property->set_edm_name( 'Gruvs' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 3 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'NRVERG' ).
    lo_primitive_property->set_edm_name( 'Nrverg' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 1 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'SSCC_18' ).
    lo_primitive_property->set_edm_name( 'Sscc18' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 1 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'INT_NKR' ).
    lo_primitive_property->set_edm_name( 'IntNkr' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 2 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'EXT_NKR' ).
    lo_primitive_property->set_edm_name( 'ExtNkr' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 2 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'TARAVAR' ).
    lo_primitive_property->set_edm_name( 'Taravar' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'Boolean' ) ##NO_TEXT.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'STSMA' ).
    lo_primitive_property->set_edm_name( 'Stsma' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 8 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'KZPGN' ).
    lo_primitive_property->set_edm_name( 'Kzpgn' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 1 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).
