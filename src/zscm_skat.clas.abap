...
CLASS zscm_skat DEFINITION
  ...
  PUBLIC SECTION.
    ...
    TYPES:
      "! <p class="shorttext synchronized">Z_I_SKATType</p>
      BEGIN OF tys_z_i_skattype,
        "! <em>Key property</em> Spras
        spras           TYPE c LENGTH 2,
        "! <em>Key property</em> Ktopl
        ktopl           TYPE c LENGTH 4,
        "! <em>Key property</em> Saknr
        saknr           TYPE c LENGTH 10,
        "! Txt20
        txt_20          TYPE c LENGTH 20,
        "! Txt50
        txt_50          TYPE c LENGTH 50,
        "! Mcod1
        ...
        "! odata.etag
        etag            TYPE string,
      END OF tys_z_i_skattype,
      "! <p class="shorttext synchronized">List of Z_I_SKATType</p>
      tyt_z_i_skattype TYPE STANDARD TABLE OF tys_z_i_skattype WITH DEFAULT KEY.
    ...
    CONSTANTS:
      ...
      BEGIN OF gcs_entity_set,
        ...
        "! Z_I_SKAT
        "! <br/> Collection of type 'Z_I_SKATType'
        z_i_skat                   TYPE /iwbep/if_cp_runtime_types=>ty_entity_set_name VALUE 'Z_I_SKAT',
    ...
    CONSTANTS:
      ...
      BEGIN OF gcs_entity_type,
        ...
        "! <p class="shorttext synchronized">Internal names for Z_I_SKATType</p>
        "! See also structure type {@link ..tys_z_i_skattype}
        BEGIN OF z_i_skattype,
          "! <p class="shorttext synchronized">Navigation properties</p>
          BEGIN OF navigation,
            "! Dummy field - Structure must not be empty
            dummy TYPE int1 VALUE 0,
  ...
  PRIVATE SECTION.
    ...
    "! <p class="shorttext synchronized">Define Z_I_SKATType</p>
    "! @raising /iwbep/cx_gateway | <p class="shorttext synchronized">Gateway Exception</p>
    METHODS def_z_i_skattype RAISING /iwbep/cx_gateway.

ENDCLASS.


CLASS zscm_skat IMPLEMENTATION.

  METHOD /iwbep/if_v4_mp_basic_pm~define.

    mo_model = io_model.
    mo_model->set_schema_namespace( 'cds_zsd_skat' ) ##NO_TEXT.
  ...
  METHOD def_z_i_skattype.
    ...
    lo_entity_type = mo_model->create_entity_type_by_struct(
                                    iv_entity_type_name       = 'Z_I_SKATTYPE'
                                    is_structure              = VALUE tys_z_i_skattype( )
                                    iv_do_gen_prim_props         = abap_true
                                    iv_do_gen_prim_prop_colls    = abap_true
                                    iv_do_add_conv_to_prim_props = abap_true ).

    lo_entity_type->set_edm_name( 'Z_I_SKATType' ) ##NO_TEXT.


    lo_entity_set = lo_entity_type->create_entity_set( 'Z_I_SKAT' ).
    lo_entity_set->set_edm_name( 'Z_I_SKAT' ) ##NO_TEXT.
    ...
    lo_primitive_property = lo_entity_type->get_primitive_property( 'TXT_20' ).
    lo_primitive_property->set_edm_name( 'Txt20' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
    lo_primitive_property->set_max_length( 20 ) ##NUMBER_OK.
    lo_primitive_property->set_is_nullable( ).

    lo_primitive_property = lo_entity_type->get_primitive_property( 'TXT_50' ).
    lo_primitive_property->set_edm_name( 'Txt50' ) ##NO_TEXT.
    lo_primitive_property->set_edm_type( 'String' ) ##NO_TEXT.
